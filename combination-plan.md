...
| hero trade | duplicate `hero standard`, minor modifications — done, phase 4 |
| specials banner | **new** — small — done, phase 4 |
...
Both types are a definition file, an entry in `modules.json`, and nothing else.
Phase 3 is why: the layouts are one line each.

| | `hero trade` | `specials banner` |
|---|---|---|
| File | `modules/hero/hero trade.js` | `modules/basic/specials banner.js` |
| Depth | 1 | 2 — a rounded container |
| Palette | `primary` — yellow for Petbarn | `red` — new, see below |
| Layout | `single_column(content)` | `single_column(content)` |
| Lines | 110, over half of them comment | 98, likewise |

**`specials banner` did not need to be a fragment.** v2.5 shipped it as
`specials banner_Petbarn.njk`: 49 lines of hand-written markup with the
container colour, the terms styling and the link to
petbarn.com.au/special-offers all typed into it. None of that had to be. The
artwork and its link come from the brief like any other image, the terms are the
terms component's own defaults, and the red is a palette. What is left is four
spacing values and a palette name, and it renders the same table v2.5's
hand-written markup did.

It is also not brand-locked as a result, where the v2.5 fragment could only ever
have been Petbarn's.

**One palette was missing.** At depth 2 a container's colour is its *palette's
background* — `setPalette()` overwrites `colour` from the palette, so
`colour: "red"` in a definition does nothing. Petbarn had no red palette, so one
was added to `libraries/colour palettes/petbarn palettes.json` carrying v2.5's
`PB Red`: `#F05842` with white text throughout. That is what makes the terms
legible without a rule, and it is now available to any module.

**Two things from v2.5's `hero trade` rule were left behind on purpose.**
`image_margin: "-1px 0px"` closed a sub-pixel seam between stacked artwork;
nothing in this engine renders margin on an image, so carrying it across would
have been a setting that reads as doing something and does nothing. The heading
sizes (h3 / 64px / h4) were copied verbatim into v2.5's rule from `hero trade
large` and applied to components its own `order` excluded — dead config in the
original, and not worth reproducing.

`trade image banner` stays out. Its v2.5 rule is an image alone, 32px padding,
24px radius, light grey — which is `text block` with one image, as section 4.2
...
positions. So there is an eighth case, `new-modules`, building both types on a
Petbarn sheet — one trade hero with artwork and legal only, a second carrying
the full component set, two specials banners, and an ordinary module after them
so the background transition is on the record too.

Its brief is generated by `tests/golden/briefs/new-modules.js` rather than saved
from Excel. The other briefs are real ones and should stay that way; this one
exists to cover types no campaign has used yet, and a committed `.xlsx` cannot
be read in a diff or edited without Excel.
...
- **`hero standard.js` ends its `style()` with an unconditional `update()`.**
  It sets `hide_transition: true`, `padding: "0px"` and `palette: "white"` on
  every hero, whatever the two branches above it decided — so the branch for
  "the only child is an image" is dead, and so is the module's own
  `palette: "primary"`. The stray indentation says debugging left behind rather
  than a decision. Found in phase 4 and not touched: the goldens have recorded
  it since phase 0, so fixing it is a visible output change on real Petbarn and
  Greencross campaigns and wants to be its own reviewable commit. `hero trade`
  was written without it.
- **An image linked the way older briefs write it renders broken.**
  `formatRichText` takes an image's link from brackets — `artwork (https://...)`
  — and `demo.xlsx` writes one as `artwork https://...`, space-separated, which
  is the v2.5 convention. Nothing parses that: the whole string lands in `src`,
  so the file is not found and no link is emitted. It is in the committed golden
