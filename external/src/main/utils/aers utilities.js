...
/*
    ...
    for nothing else. The previous generation of the brief template carries one header
...
function find_header_row(ws, required_column, description) {
    ...
    for (let R = 0; R <= last_row; R++) {
        if (keys_in_row(ws, R, range).includes(required_column)) {
            ...
            /*
                Stop at the first row that is not a header. The current brief template
...
/**
 * The column keys of a sheet's header row, camelCased as sheet_to_objects
 * produces them.
 *
 * Which of the two brief templates a sheet was written from is a question about
 * its header, not about its data -- and it has to be asked before
 * sheet_to_objects deletes that header. See processing/brief_format.js.
 *
 * @param {object} ws              the worksheet
 ...
 * @returns {string[]} the header's column keys, in column order
 */
function header_keys(ws, required_column, description) {
    const header = find_header_row(ws, required_column, description);
    return keys_in_row(ws, header, XLSX.utils.decode_range(ws["!ref"]));
}
